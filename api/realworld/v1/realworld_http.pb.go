...
func RegisterRealWorldHTTPServer(s *http.Server, srv RealWorldHTTPServer) {
	...
	r.POST("/api/users", _RealWorld_RegisterRequest0_HTTP_Handler(srv))
...
func _RealWorld_RegisterRequest0_HTTP_Handler(srv RealWorldHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in LoginRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, "/realworld.v1.RealWorld/RegisterRequest")
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.RegisterRequest(ctx, req.(*LoginRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
...
type RealWorldHTTPClient interface {
	...
	RegisterRequest(ctx context.Context, req *LoginRequest, opts ...http.CallOption) (rsp *UserReply, err error)
...
func (c *RealWorldHTTPClientImpl) RegisterRequest(ctx context.Context, in *LoginRequest, opts ...http.CallOption) (*UserReply, error) {
	var out UserReply
	pattern := "/api/users"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation("/realworld.v1.RealWorld/RegisterRequest"))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "POST", path, in, &out, opts...)
	if err != nil {
		return nil, err
